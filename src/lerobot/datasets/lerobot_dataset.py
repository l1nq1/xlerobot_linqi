...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def load_hf_dataset(self) -> datasets.Dataset:
        ...
        # Get features from metadata to avoid compatibility issues with parquet metadata
        # (e.g., 'List' type in old parquet files is not supported by newer datasets versions)
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        return {
            key: torch.stack(tuple(self.hf_dataset.select(q_idx)[key]))
